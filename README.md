...
```mermaid
flowchart LR
    A["Binance WS/REST\ndepth@100ms + trade"] --> B["acquisition.py\nraw JSONL capture"]
    B --> C["orderbook.py\nresync_and_replay"]
    C --> D["Parquet\nbook states"]
    D --> E["features.py\nevent OFI, windows,\nfwd-return targets"]
    E --> F["modeling.py\nOLS vs. train-mean\nbaseline"]
    F --> G["evaluation.py\nwalk-forward, OOS R²,\nNewey-West, bootstrap"]
    G --> H["paper/\nwrite-up (pending)"]
...
|---|---|---|
| 1 | Acquisition & book reconstruction | [`acquisition.py`](src/acquisition.py), [`orderbook.py`](src/orderbook.py) |
| 2 | OFI features & forward targets | [`features.py`](src/features.py) |
| 3 | Regression models | [`modeling.py`](src/modeling.py) |
| 4 | Walk-forward validation & inference | [`evaluation.py`](src/evaluation.py) |
| 5 | Results & write-up | `paper/` — not started yet |
...
and writes a results table; [`scripts/plot_results.py`](scripts/plot_results.py) turns that table
into the headline **R²-vs-horizon decay curve** (the figure that most directly tests the
hypothesis — predictive power should be highest at 1s and decay toward zero by ~15s).

## Figures

*Populated once the full multi-day capture and study run complete — see [Status](#status--roadmap).
Both figures below are generated by code already in this repo, not manually created, so they'll
reflect whatever the data actually shows:*

- `reports/sanity/mid_price_full.png` / `mid_price_zoom_5min.png` — generated by
  [`scripts/sanity_check.py`](scripts/sanity_check.py) from the real capture.
- `paper/figures/r2_decay.png` — generated by [`scripts/plot_results.py`](scripts/plot_results.py)
  from `reports/results.csv`.
...
# 6. Plot the headline R²-vs-horizon decay curve
python -m scripts.plot_results --results reports/results.csv --out paper/figures/r2_decay.png
```
